...
$usingDefaultPort = true || (!isset($_SERVER["HTTPS"]) && $_SERVER["SERVER_PORT"] === 80) || (isset($_SERVER["HTTPS"]) && $_SERVER["SERVER_PORT"] === 443);
    ...
    foreach ($sourceLines as $line) {
        ...
        if (preg_match("/@import\s+url/i", $line)) {
            ...
            $normalizedLines[] = preg_replace_callback(
                    ...
                    "/(@import\s+)([^;\s]+)([\s;])/i", function($matches) use ($baseURL) {
...
$header_blacklist_pattern = "/^Content-Length|^Transfer-Encoding|^Content-Encoding.*gzip|^X-Frame-Options|^Content-Security-Policy|^content-security-policy/i";
...
if (stripos($contentType, "text/html") !== false) {
    ...
    foreach ($proxifyAttributes as $attrName) {
        ...
        foreach ($xpath->query("//*[@" . $attrName . "][not(@target)]") as $element) { //For every element with the given attribute...            
            ...
            if (strpos($attrContent, 'https://nasa.gov') === 0 || strpos($attrContent, 'https://www.nasa.gov') === 0 || strpos($attrContent, 'http://nasa.gov') === 0 || strpos($attrContent, 'http://www.nasa.gov') === 0) {
    ...
    //$response = str_replace('https://nasa3d.arc.nasa.gov/', 'http://cypretex.com/proxy?https://nasa3d.arc.nasa.gov/', $response);

    //$responseBody= utf8_encode($responseBody);

    $responseBody = str_replace('top.location=self.document.location;', '', $responseBody);

    $responseBody = str_replace('$(\'body\').html(\'<div>redirecting...</div>\');', '', $responseBody);

    $responseBody = preg_replace('#href="/([A-zA-Z0-9]+)"#', PROXY_PREFIX . '/$1', $responseBody);

    $responseBody = preg_replace('#src="/([A-zA-Z0-9]+)"#', PROXY_PREFIX . '/$1', $responseBody);

    $responseBody = preg_replace("#window\.location='([A-zA-Z0-9]+)'#", "window\.location='" . PROXY_PREFIX . "$1'", $responseBody);

    // $responseBody = mb_convert_encoding($utf8, 'ISO-8859-1', 'UTF-8');

    // $responseBody = str_replace('script', '', $responseBody);

    //$responseBody = preg_replace( '/[^[:print:]]/', '',$responseBody);
